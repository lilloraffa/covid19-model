...
class ModelStats:
    ...
    def printKpis(self, date):
        self.printKpi(date, 'Igc_cum', "Tot Infettati")
        self.printKpi(date, 'Igc', "Attualmente Infetti")
        self.printKpi(date, 'Igci_t', "Attualmente in Intensiva")
        self.printKpi(date, 'Gc_cum', "Tot Guariti")
        self.printKpi(date, 'M_cum', "Tot Morti")
        print()
        self.printKpi(date, 'Igc_cum_pinc', "Incremento % Infettati", num_format=".3f", bperc = True)
        self.printKpi(date, 'ratio_Gc_Igc', "Tasso % mortalità", num_format=".3f", bperc = True)
        self.printKpi(date, 'ratio_M_Igc', "Tasso % guarigione conosciuta", num_format=".3f", bperc = True)
        print()
        self.printKpi(date, 'ratio_Gccum_Igccum', "% Guariti su Tot", num_format=".3f", bperc = True)
        self.printKpi(date, 'ratio_Mcum_Igccum', "% Morti su Tot", num_format=".3f", bperc = True)
        self.printKpi(date, 'ratio_Igci_Igc', "% Terapia intensiva su Infetti", num_format=".3f", bperc = True)
        self.printKpi(date, 'ratio_Igcn_Igc', "% Terapia non int. su Infetti", num_format=".3f", bperc = True)
        self.printKpi(date, 'ratio_Ias_Igc', "% Infetti scon. su Infetti", num_format=".3f", bperc = True)
        print()
        print()
